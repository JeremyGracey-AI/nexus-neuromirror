...
export function UploadPage() {
  ...
  // Fallbacks used only until /health loads. Vercel caps serverless request
  // bodies at ~4.5 MB, so the server enforces a 4 MB upload limit.
  const maxMb = health?.max_upload_mb ?? 4;
  const maxBytes = health?.max_upload_bytes ?? 4 * 1024 * 1024;
      ...
      <div className="flex items-start gap-2 rounded-lg border border-border bg-surface-alt px-4 py-3 text-sm">
        ...
        <div>
          ...
          <p className="mt-0.5 text-text-muted">
            ...
            sanitized, contents are never logged, and uploads are capped at {maxMb} MB (Vercel
            rejects serverless request bodies over ~4.5 MB).
          </p>
        </div>
      </div>
